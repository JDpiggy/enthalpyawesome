document.addEventListener('DOMContentLoaded', () => {
    ...
    const POWERUP_SIZE = 50; const POWERUP_SPAWN_CHANCE = 0.0085;
    ...
    class Rocket {
        ...
        draw() { const charImg = this.character.imageObj; if (this.character.isReady && charImg.complete && charImg.naturalWidth !== 0) { ctx.drawImage(charImg, this.x, this.y, this.width, this.height); } else { ctx.fillStyle = 'purple'; ctx.fillRect(this.x, this.y, this.width, this.height); } if (this.shieldActive) { ctx.strokeStyle = 'rgba(0, 255, 255, 0.8)'; ctx.lineWidth = 5; ctx.beginPath(); const shieldRadius = Math.max(this.width, this.height) * 0.8; ctx.arc(this.x + this.width / 2, this.y + this.height / 2, shieldRadius, 0, Math.PI * 2); ctx.stroke(); } }
    }
    class Obstacle {
        constructor(x, initialGapY, movesVertically, type) { this.x = x; this.type = type; const typeProps = OBSTACLE_TYPES[this.type]; this.image = typeProps.img; this.visualWidth = typeProps.visualWidth; this.effectiveWidth = typeProps.effectiveWidth; this.hitboxInsetX = typeProps.hitboxInsetX; this.hitboxInsetYGapEdge = typeProps.hitboxInsetYGapEdge; this.initialGapY = initialGapY; this.currentGapY = initialGapY; this.movesVertically = movesVertically; this.verticalDirection = Math.random() > 0.5 ? 1 : -1; this.passed = false; this.topPart = { y: 0, height: 0 }; this.bottomPart = { y: 0, height: 0 }; this._calculateDimensions(); }
        ...
        update() { this.x -= gameSpeed; if (this.movesVertically) { const moveAmount = OBSTACLE_VERTICAL_SPEED * this.verticalDirection; let newGapCenter = this.currentGapY + moveAmount; const minBase = this.initialGapY - OBSTACLE_VERTICAL_MOVEMENT_MAX_OFFSET; const maxBase = this.initialGapY + OBSTACLE_VERTICAL_MOVEMENT_MAX_OFFSET; const screenMin = OBSTACLE_GAP / 2 + MIN_OBSTACLE_SEGMENT_HEIGHT + 10; const screenMax = GAME_HEIGHT - (OBSTACLE_GAP / 2 + MIN_OBSTACLE_SEGMENT_HEIGHT + 10); const finalMin = Math.max(minBase, screenMin); const finalMax = Math.min(maxBase, screenMax); if (newGapCenter > finalMax || newGapCenter < finalMin) { this.verticalDirection *= -1; newGapCenter = Math.max(finalMin, Math.min(finalMax, newGapCenter)); } this.currentGapY = newGapCenter; } this._calculateDimensions(); }
        draw() { if (!this.image || !this.image.isReady) return; const drawX = this.x - (this.visualWidth - this.effectiveWidth) / 2; if (this.topPart.height > 0) { ctx.save(); ctx.translate(drawX, this.topPart.y + this.topPart.height); ctx.scale(1, -1); ctx.drawImage(this.image, 0, 0, this.visualWidth, this.topPart.height); ctx.restore(); } if (this.bottomPart.height > 0) { ctx.drawImage(this.image, drawX, this.bottomPart.y, this.visualWidth, this.bottomPart.height); } }
    }
    class PowerUp {
        constructor(x, y, type) { this.x = x; this.y = y; this.size = POWERUP_SIZE; this.type = type; this.collected = false; } update() { this.x -= gameSpeed; }
        draw() { if (this.collected) return; const centerX = this.x + this.size / 2; const centerY = this.y + this.size / 2; if (this.type === 'shield') { ctx.beginPath(); ctx.arc(centerX, centerY, this.size / 2, 0, Math.PI * 2); ctx.fillStyle = 'rgba(0, 220, 255, 0.9)'; ctx.fill(); ctx.strokeStyle = 'rgba(20,20,20,0.7)'; ctx.lineWidth = 2; ctx.stroke(); const symbol = 'S'; const font = `bold ${this.size * 0.7}px 'Bangers', cursive`; ctx.fillStyle = '#1e272e'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.font = font; ctx.fillText(symbol, centerX, centerY + this.size * 0.08); } else if (this.type === 'fuel') { if (fuelPowerUpImg.isReady) { ctx.drawImage(fuelPowerUpImg, this.x, this.y, this.size, this.size); } else { ctx.beginPath(); ctx.arc(centerX, centerY, this.size / 2, 0, Math.PI * 2); ctx.fillStyle = 'rgba(255, 190, 0, 0.9)'; ctx.fill(); const symbol = 'F'; const font = `bold ${this.size * 0.7}px 'Bangers', cursive`; ctx.fillStyle = '#1e272e'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.font = font; ctx.fillText(symbol, centerX, centerY + this.size * 0.08); } } }
        applyEffect(rocketInstance) { playSound(sounds.powerup); if (this.type === 'shield') { rocketInstance.shieldActive = true; rocketInstance.shieldTimer = SHIELD_DURATION; } else if (this.type === 'fuel') { rocketInstance.fuel = MAX_FUEL; } this.collected = true; for (let i = 0; i < 20; i++) { particles.push(new Particle(this.x + this.size / 2, this.y + this.size / 2, 'collect')); } }
    }
    class Particle {
        constructor(x, y, type) { this.x = x; this.y = y; this.type = type; this.size = Math.random() * (type === 'explosion' ? 10 : (type === 'thrust' ? 8 : 7)) + 3; this.initialLife = (type === 'explosion' ? 70 : (type === 'thrust' ? 30 : 40)) + Math.random() * 30; this.life = this.initialLife; const angle = Math.random() * Math.PI * 2; let speed = Math.random() * (type === 'explosion' ? 10 : (type === 'collect' ? 5 : 3)) + 1; if (type === 'thrust') { const r = Math.floor(Math.random() * 50) + 100; const g = Math.floor(Math.random() * 40) + 60; const b = Math.floor(Math.random() * 30) + 20; this.color = `rgba(${r}, ${g}, ${b}, ${Math.random() * 0.4 + 0.4})`; this.velocityX = (Math.random() - 0.5) * 2.5; this.velocityY = Math.random() * 2.0 + 1.0; this.size = Math.random() * 8 + 4; } else { this.velocityX = Math.cos(angle) * speed; this.velocityY = Math.sin(angle) * speed; if (type === 'explosion') { this.color = `rgba(${Math.floor(Math.random() * 255)}, ${Math.floor(Math.random() * 120)}, 0, 0.8)`; } else { this.color = `rgba(255, 230, ${Math.random() > 0.5 ? 50 : 150}, 0.8)`; } } }
    ...
    function playSound(sound) { if (sound && sound.src && sound.readyState >= HTMLMediaElement.HAVE_CURRENT_DATA) { sound.currentTime = 0; sound.play().catch(error => console.warn(`Sound play failed for ${sound.src.split('/').pop()}:`, error)); } else if (sound && sound.src) { console.warn(`Sound ${sound.src.split('/').pop()} not ready. State: ${sound.readyState}, Network: ${sound.networkState}`); if (sound.networkState === HTMLMediaElement.NETWORK_NO_SOURCE || sound.networkState === HTMLMediaElement.NETWORK_EMPTY) { if (sound.src) sound.load(); } } }
    ...
    function drawStartScreenCharacter(bobOffset) { const gameChar = getCurrentGameCharacter(); if (!gameChar || !gameChar.isReady || !ctx) return; const charImg = gameChar.imageObj; const scaleFactor = 2.8; const charWidth = ROCKET_WIDTH * scaleFactor; const charHeight = ROCKET_HEIGHT * scaleFactor; const charX = GAME_WIDTH * 0.80 - charWidth / 2; const charYBase = GAME_HEIGHT / 2 - charHeight / 2; const charY = charYBase + bobOffset; if (charImg.complete && charImg.naturalWidth !== 0) ctx.drawImage(charImg, charX, charY, charWidth, charHeight); else { ctx.fillStyle = 'grey'; ctx.fillRect(charX, charY, charWidth, charHeight); } }
    function startScreenAnimationLoop() { if (gameState !== 'START' || !isStartScreenLoopRunning || startScreen.style.display === 'none') { isStartScreenLoopRunning = false; return; } if (ctx) ctx.clearRect(0, 0, GAME_WIDTH, GAME_HEIGHT); startScreenAnimFrame++; const bobOffset = Math.sin(startScreenAnimFrame * startScreenCharBobSpeed) * startScreenCharYOffsetMax; drawStartScreenCharacter(bobOffset); requestAnimationFrame(startScreenAnimationLoop); }

    function loadGameData() { coins = parseInt(localStorage.getItem('flappyLaliCoins_v2')) || 0; highScore = parseInt(localStorage.getItem('flappyLaliFartV2_hs')) || 0; currentSelectedCharacterId = localStorage.getItem('flappyLaliSelectedChar_v2') || 'lali_classic'; shopPreviewCharacterId = currentSelectedCharacterId; const unlockedChars = JSON.parse(localStorage.getItem('flappyLaliUnlockedChars_v2')); if (unlockedChars && Array.isArray(unlockedChars)) { charactersData.forEach(char => { char.unlocked = unlockedChars.includes(char.id); }); } const classicChar = getCharacterById('lali_classic'); if (classicChar) classicChar.unlocked = true; const selected = getCharacterById(currentSelectedCharacterId); if (!selected || !selected.unlocked) { currentSelectedCharacterId = 'lali_classic'; shopPreviewCharacterId = 'lali_classic'; localStorage.setItem('flappyLaliSelectedChar_v2', currentSelectedCharacterId); } updateCoinDisplay(); }
    ...
    function renderCharacterShop() { if (!shopPanelLeft) return; shopPanelLeft.innerHTML = ''; charactersData.forEach(char => { const slot = document.createElement('div'); slot.classList.add('character-slot'); if (char.id === shopPreviewCharacterId) slot.classList.add('selected-in-shop'); const img = new Image(); img.src = char.imageSrc; img.alt = char.name; if (!char.isReady || !char.imageObj.complete || char.imageObj.naturalWidth === 0) img.classList.add('not-ready'); slot.appendChild(img); const infoDiv = document.createElement('div'); infoDiv.classList.add('char-info-shop'); const nameP = document.createElement('p'); nameP.classList.add('char-name'); nameP.textContent = char.name; infoDiv.appendChild(nameP); if (char.unlocked) { const statusP = document.createElement('p'); statusP.classList.add('char-status'); statusP.textContent = (char.id === currentSelectedCharacterId) ? "Equipped" : "Owned"; infoDiv.appendChild(statusP); } else { const priceP = document.createElement('p'); priceP.classList.add('char-price'); priceP.textContent = `Price: ${char.price}`; infoDiv.appendChild(priceP); } slot.appendChild(infoDiv); const buttonContainer = document.createElement('div'); buttonContainer.classList.add('shop-button-container'); if (char.unlocked) { if (char.id !== currentSelectedCharacterId) { const equipButton = document.createElement('button'); equipButton.textContent = "Equip"; equipButton.onclick = (e) => { e.stopPropagation(); equipCharacter(char.id); }; buttonContainer.appendChild(equipButton); } } else { const buyButton = document.createElement('button'); buyButton.textContent = "Buy"; if (coins < char.price) buyButton.disabled = true; buyButton.onclick = (e) => { e.stopPropagation(); buyCharacter(char.id); }; buttonContainer.appendChild(buyButton); } slot.appendChild(buttonContainer); slot.onclick = () => updateShopPreview(char.id); shopPanelLeft.appendChild(slot); }); }
    function updateShopPreview(charId) { shopPreviewCharacterId = charId; const char = getCharacterById(charId); if (!char) return; if (shopCharacterPreviewImage) { if (char.isReady && char.imageObj.complete && char.imageObj.naturalWidth > 0) { shopCharacterPreviewImage.src = char.imageObj.src; shopCharacterPreviewImage.classList.remove('not-ready'); } else { shopCharacterPreviewImage.src = ''; shopCharacterPreviewImage.classList.add('not-ready'); } } if (shopCharacterName) shopCharacterName.textContent = char.name; if (shopCharacterPriceStatus) { if (char.unlocked) { shopCharacterPriceStatus.textContent = (char.id === currentSelectedCharacterId) ? "Currently Equipped" : "Owned"; shopCharacterPriceStatus.className = 'char-status owned'; } else { shopCharacterPriceStatus.textContent = `Price: ${char.price} Coins`; shopCharacterPriceStatus.className = 'char-status'; } } const slots = shopPanelLeft.querySelectorAll('.character-slot'); slots.forEach(s => { const slotCharName = s.querySelector('.char-name').textContent; const slotChar = charactersData.find(c => c.name === slotCharName); if (slotChar && slotChar.id === charId) s.classList.add('selected-in-shop'); else s.classList.remove('selected-in-shop'); }); }
    function equipCharacter(charId) { const charToEquip = getCharacterById(charId); if (charToEquip && charToEquip.unlocked) { currentSelectedCharacterId = charId; saveCharacterData(); renderCharacterShop(); updateShopPreview(shopPreviewCharacterId); } }
    function buyCharacter(charId) { const charToBuy = getCharacterById(charId); if (charToBuy && !charToBuy.unlocked && coins >= charToBuy.price) { coins -= charToBuy.price; charToBuy.unlocked = true; playSound(sounds.purchase); saveCoins(); saveCharacterData(); updateCoinDisplay(); renderCharacterShop(); updateShopPreview(charId); } }

    const redeemCodes = { "imjaron": { description: "Unlocks all Lali characters!", action: () => { let unlockedSomething = false; charactersData.forEach(char => { if (!char.unlocked) { char.unlocked = true; unlockedSomething = true; } }); if (unlockedSomething) { saveCharacterData(); if (shopScreen.style.display !== 'none') { renderCharacterShop(); updateShopPreview(shopPreviewCharacterId); } return true; } return false; } } };
    function handleRedeemCode() { if (!redeemCodeInput || !redeemStatusMessage) return; const enteredCode = redeemCodeInput.value.trim().toLowerCase(); redeemCodeInput.value = ''; if (redeemCodes[enteredCode]) { const codeEffect = redeemCodes[enteredCode]; const success = codeEffect.action(); if (success) { playSound(sounds.purchase); redeemStatusMessage.textContent = codeEffect.description || "Code redeemed successfully!"; redeemStatusMessage.className = 'success'; } else { redeemStatusMessage.textContent = "Code applied, but no new changes."; redeemStatusMessage.className = 'success'; } } else { redeemStatusMessage.textContent = "Invalid code. Please try again."; redeemStatusMessage.className = 'error'; } redeemStatusMessage.style.display = 'block'; setTimeout(() => { if (redeemStatusMessage) redeemStatusMessage.style.display = 'none'; }, 4000); }

    function initGame() { if (gameState !== 'LOADING') loadGameData(); rocket = null; obstacles = []; powerUps = []; particles = []; score = 0; frame = 0; gameSpeed = OBSTACLE_SPEED_INITIAL; if (startScreen) startScreen.style.display = 'flex'; if (gameOverScreen) gameOverScreen.style.display = 'none'; if (shopScreen) shopScreen.style.display = 'none'; if (redeemStatusMessage) redeemStatusMessage.style.display = 'none'; if (startButton) startButton.disabled = assetsLoaded < assetsToLoad; if (shopButton) shopButton.disabled = assetsLoaded < assetsToLoad; if (redeemCodeButton) redeemCodeButton.disabled = assetsLoaded < assetsToLoad; updateUI(null); if (ctx) ctx.clearRect(0, 0, GAME_WIDTH, GAME_HEIGHT); isStartScreenLoopRunning = false; if (gameState === 'START') { const gameChar = getCurrentGameCharacter(); if (gameChar && gameChar.isReady) { isStartScreenLoopRunning = true; startScreenAnimationLoop(); } if (backgroundMusic.isReady && backgroundMusic.paused) { backgroundMusic.play().catch(e => console.warn("BG Music autoplay likely blocked on init.", e)); } } }
    function startGame() { isStartScreenLoopRunning = false; if (ctx) ctx.clearRect(0, 0, GAME_WIDTH, GAME_HEIGHT); gameState = 'PLAYING'; if (startScreen) startScreen.style.display = 'none'; if (shopScreen) shopScreen.style.display = 'none'; if (gameOverScreen) gameOverScreen.style.display = 'none'; rocket = new Rocket(); obstacles = []; powerUps = []; particles = []; score = 0; frame = 0; gameSpeed = OBSTACLE_SPEED_INITIAL; canSpawnEmergencyBeans = true; emergencyBeansCooldownTimer = 0; updateUI(rocket); if (backgroundMusic.isReady && backgroundMusic.paused) { backgroundMusic.play().catch(e => console.error("Error playing background music:", e)); } gameLoop(); }
    function gameOver() { playSound(sounds.hit); gameState = 'GAMEOVER'; const earnedThisGame = score; coins += earnedThisGame; saveCoins(); updateCoinDisplay(); if (coinsEarnedDisplay) coinsEarnedDisplay.textContent = earnedThisGame; if (rocket) { for (let i = 0; i < 50; i++) { particles.push(new Particle(rocket.x + rocket.width / 2, rocket.y + rocket.height / 2, 'explosion')); } rocket = null; } if (score > highScore) { highScore = score; saveHighScore(); if (newHighScoreTextGameOver) newHighScoreTextGameOver.style.display = 'block'; } else { if (newHighScoreTextGameOver) newHighScoreTextGameOver.style.display = 'none'; } if (finalScoreDisplay) finalScoreDisplay.textContent = score; if (gameOverScreen) gameOverScreen.style.display = 'flex'; updateUI(null); }
    ...
    function generateObstacles() { if (frame % Math.floor(OBSTACLE_SPACING / gameSpeed) === 0) { const obstacleTypeKeys = Object.keys(OBSTACLE_TYPES); const randomType = obstacleTypeKeys[Math.floor(Math.random() * obstacleTypeKeys.length)]; const minGapCenter = OBSTACLE_GAP / 2 + MIN_OBSTACLE_SEGMENT_HEIGHT + 20; const maxGapCenter = GAME_HEIGHT - (OBSTACLE_GAP / 2 + MIN_OBSTACLE_SEGMENT_HEIGHT + 20); const range = maxGapCenter - minGapCenter; let initialGapY = (range > 0) ? (Math.random() * range + minGapCenter) : (GAME_HEIGHT / 2); const movesVertically = Math.random() < 0.4; obstacles.push(new Obstacle(GAME_WIDTH, initialGapY, movesVertically, randomType)); } obstacles = obstacles.filter(obs => obs.x + obs.visualWidth > 0); }
    function generatePowerUps() { if (Math.random() < POWERUP_SPAWN_CHANCE && powerUps.length < 3) { const type = Math.random() < 0.4 ? 'shield' : 'fuel'; const y = Math.random() * (GAME_HEIGHT - POWERUP_SIZE - 150) + 75; const x = GAME_WIDTH + Math.random() * 200; powerUps.push(new PowerUp(x, y, type)); } powerUps = powerUps.filter(pu => pu.x + pu.size > 0 && !pu.collected); }
    function trySpawnEmergencyBeans() { if (rocket && rocket.fuel < (MAX_FUEL * (LOW_FUEL_THRESHOLD_PERCENT / 100)) && canSpawnEmergencyBeans) { const existingFuelPU = powerUps.find(pu => pu.type === 'fuel'); if (!existingFuelPU) { let spawnX = GAME_WIDTH * 0.8; const nearRocketY = rocket.y + (Math.random() - 0.5) * 100; const clampedY = Math.max(POWERUP_SIZE / 2, Math.min(GAME_HEIGHT - POWERUP_SIZE * 1.5, nearRocketY)); const nextObs = obstacles.find(o => o.x + o.effectiveWidth > rocket.x + rocket.width); if (nextObs) spawnX = nextObs.x + nextObs.effectiveWidth + Math.random() * OBSTACLE_SPACING * 0.3 + 50; else if (obstacles.length > 0 && obstacles[obstacles.length - 1].x + obstacles[obstacles.length - 1].effectiveWidth > 0) { spawnX = obstacles[obstacles.length - 1].x + obstacles[obstacles.length - 1].effectiveWidth + Math.random() * OBSTACLE_SPACING * 0.3 + 50; } spawnX = Math.max(spawnX, rocket.x + GAME_WIDTH * 0.3); spawnX = Math.min(spawnX, GAME_WIDTH * 1.5); powerUps.push(new PowerUp(spawnX, clampedY, 'fuel')); canSpawnEmergencyBeans = false; emergencyBeansCooldownTimer = EMERGENCY_BEANS_COOLDOWN_FRAMES; } } if (emergencyBeansCooldownTimer > 0) { emergencyBeansCooldownTimer--; if (emergencyBeansCooldownTimer <= 0) canSpawnEmergencyBeans = true; } }
    function checkCollisions() { if (!rocket || gameState !== 'PLAYING') return; if (rocket.y + rocket.height >= GAME_HEIGHT) { rocket.y = GAME_HEIGHT - rocket.height; rocket.velocityY = 0; if (!rocket.shieldActive) { gameOver(); return; } else { rocket.velocityY = FLAP_STRENGTH * 0.3; playSound(sounds.hit); } } for (let obs of obstacles) { const rocketRect = { x: rocket.x, y: rocket.y, width: rocket.width, height: rocket.height }; const obsCollisionX = obs.x + obs.hitboxInsetX; const obsCollisionWidth = obs.effectiveWidth - 2 * obs.hitboxInsetX; const topPartRect = { x: obsCollisionX, y: obs.topPart.y, width: obsCollisionWidth, height: obs.topPart.height - obs.hitboxInsetYGapEdge }; if (topPartRect.height < 0) topPartRect.height = 0; if (!rocket.shieldActive && rocketRect.x < topPartRect.x + topPartRect.width && rocketRect.x + rocketRect.width > topPartRect.x && rocketRect.y < topPartRect.y + topPartRect.height && rocketRect.y + rocketRect.height > topPartRect.y) { gameOver(); return; } const bottomPartRect = { x: obsCollisionX, y: obs.bottomPart.y + obs.hitboxInsetYGapEdge, width: obsCollisionWidth, height: obs.bottomPart.height - obs.hitboxInsetYGapEdge }; if (bottomPartRect.height < 0) bottomPartRect.height = 0; if (!rocket.shieldActive && rocketRect.x < bottomPartRect.x + bottomPartRect.width && rocketRect.x + rocketRect.width > bottomPartRect.x && rocketRect.y < bottomPartRect.y + bottomPartRect.height && rocketRect.y + rocketRect.height > bottomPartRect.y) { gameOver(); return; } if (!obs.passed && obs.x + obs.effectiveWidth < rocket.x) { obs.passed = true; score++; playSound(sounds.score); gameSpeed += 0.02; } } for (let pu of powerUps) { if (!pu.collected && rocket.x < pu.x + pu.size && rocket.x + rocket.width > pu.x && rocket.y < pu.y + pu.size && rocket.y + rocket.height > pu.y) pu.applyEffect(rocket); } }
    function updateGameObjects() { if (gameState !== 'PLAYING') return; if (rocket) rocket.update(); obstacles.forEach(obs => obs.update()); powerUps.forEach(pu => pu.update()); particles = particles.filter(p => p.life > 0); particles.forEach(p => p.update()); if (backgroundImg.isReady) { backgroundX -= gameSpeed * BACKGROUND_SCROLL_SPEED_FACTOR; if (backgroundX <= -backgroundImg.width) { backgroundX += backgroundImg.width; } } }
    function drawGameObjects() { if (!ctx) return; ctx.clearRect(0, 0, GAME_WIDTH, GAME_HEIGHT); if (backgroundImg.isReady && backgroundImg.complete && backgroundImg.naturalWidth > 0) { ctx.drawImage(backgroundImg, backgroundX, 0, backgroundImg.width, GAME_HEIGHT); if (backgroundX < 0) { ctx.drawImage(backgroundImg, backgroundX + backgroundImg.width, 0, backgroundImg.width, GAME_HEIGHT); } } else { ctx.fillStyle = '#87CEEB'; ctx.fillRect(0, 0, GAME_WIDTH, GAME_HEIGHT); } obstacles.forEach(obs => obs.draw()); powerUps.forEach(pu => pu.draw()); if (gameState === 'PLAYING' && rocket) rocket.draw(); particles.forEach(p => p.draw()); }
    ...
    function updateUI(currentRocket) { if (scoreDisplay) scoreDisplay.textContent = `Score: ${score}`; if (highScoreDisplay) highScoreDisplay.textContent = `High Score: ${highScore}`; updateCoinDisplay(); let fuelSource = currentRocket || (rocket ? rocket : { fuel: MAX_FUEL }); if (fuelBar) { if (fuelSource) { const fuelPercentage = (fuelSource.fuel / MAX_FUEL) * 100; fuelBar.style.width = `${fuelPercentage}%`; if (fuelPercentage < LOW_FUEL_THRESHOLD_PERCENT) fuelBar.style.backgroundColor = '#d63031'; else if (fuelPercentage < 50) fuelBar.style.backgroundColor = '#fdcb6e'; else fuelBar.style.backgroundColor = '#e17055'; } else { fuelBar.style.width = '100%'; fuelBar.style.backgroundColor = '#e17055'; } } }
